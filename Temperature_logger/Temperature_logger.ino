...
void loop() {
  ...
  if (timeUNIX != 0) {
    ...
    if (currentMillis - prevTemp > DS_delay && tmpRequested) { // 750 ms after requesting the temperature
      ...
      tmpRequested = false;
      float temp = DHT11.temperature;  // Get the temperature from the sensor
...
void startWiFi() { // Try to connect to some given access points. Then wait for a connection
  wifiMulti.addAP("Wifi van de buren", "Syntax1337");   // add Wi-Fi networks you want to connect to
  wifiMulti.addAP("Wifi van de buren Boven", "Syntax420");
